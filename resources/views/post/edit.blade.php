...
@section('content')
  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center px-4 py-8">
    ...
    <form method="POST" action="{{ route('post.update') }}" enctype="multipart/form-data" class="space-y-5">
      ...
      <!-- Title Input -->
      <div>
      <label for="title" class="block text-sm font-medium text-gray-700 dark:text-gray-100">
        {{ __('post.posttitle') }}
      </label>
      <input type="text" id="title" name="title" class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm 
       focus:ring-blue-500 focus:border-blue-500 sm:text-sm 
       bg-white dark:bg-gray-800 
       text-gray-900 dark:text-gray-100
       placeholder-gray-400 dark:placeholder-gray-400
       transition-colors duration-200" value="{{ old('title', $post->post_title) }}" required>
      </div>
      ...
      <a href="{{ route('post.index', [$groupid, $post->id]) }}"
